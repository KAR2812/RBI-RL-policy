...
def main():
    ...
    # Load dataset
    rbi_filepath = config.paths.resolve(config.paths.rbi_excel)
    if not os.path.exists(rbi_filepath):
        print(f"Error: Historical dataset not found at {rbi_filepath}")
        sys.exit(1)

    print("[Historical] Loading DBIE Historical Dataset...")
    df = load_rbi_historical_data(rbi_filepath)
    ...
    plot_dir = config.paths.resolve(config.paths.plots_dir)
    ...
    log_dir = config.paths.resolve(config.paths.logs_dir)
    ...
    for policy_name, m in results.items():
        ...
        print(f"\n[Outputs] {policy_name} | historical_rbib")
    ...
    ax.set_title('PPO vs Taylor Rule Parameter Comparison (Historical CPI Dataset)')
